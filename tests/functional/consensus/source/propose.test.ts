...
import { getLastCommit, getValidators, prepareNodeValidators, snoozeForBlock } from "./utilities.js";
...
describe<{
	...
	it("#single propose - should forge 3 blocks with all validators signing", async ({ nodes, validators }) => {
		...
		assert.equal((await getLastCommit(nodes[0])).block.data.proposer, validators[3].address);
