...
const main = async () => {
	let { corePath, coreData, coreNetwork } = await prompts([
		{
			type: "text",
			name: "corePath",
		...
		},
		{
			...
			validate: (value) =>
				["mainnet", "devnet", "testnet"].includes(value) ? true : `${value} is not a valid network.`,
			choices: [
	...
	if (fs.existsSync(`${paths.cache.new}/transaction-pool-${coreNetwork}.sqlite`)) {
		fs.moveSync(
			`${paths.cache.new}/transaction-pool-${coreNetwork}.sqlite`,
			`${paths.data.new}/transaction-pool.sqlite`,
		);
	}
	...
	const requiredFiles = [
		{
			...
			original: `${corePath}/packages/core/source/config/${coreNetwork}/delegates.json`,
		},
		{
			copy: `${paths.config.new}/peers.json`,
			original: `${corePath}/packages/core/source/config/${coreNetwork}/peers.json`,
		},
		{
			copy: `${paths.config.new}/plugins.js`,
			original: `${corePath}/packages/core/source/config/${coreNetwork}/plugins.js`,
		},
	];
