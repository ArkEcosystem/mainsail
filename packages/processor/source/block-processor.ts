	...
	public async process(unit: Contracts.Processor.ProcessableUnit): Promise<Contracts.Processor.BlockProcessorResult> {
		...
		try {
			...
			for (const [index, transaction] of block.transactions.entries()) {
				...
				const pad = (string_: string | number): string => string_.toString().padStart(3, " ");
				console.log(
					`Processed transaction (${pad(index)}) ${transaction.hash} with status ${receipt.status} and gas used ${receipt.gasUsed}`,
				);
