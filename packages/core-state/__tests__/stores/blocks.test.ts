...
describe("BlockStore", () => {
	...
	it("should fail to push a block if its height is not 1 and there is no last block", () => {
		...
		expect(() => store.set({ data: { height: 3, id: "3" } } as Crypto.IBlock)).toThrow();
	});

	it("should fail to push a block if it does not contain an id", () => {
		const store = new BlockStore(2);

		expect(() => store.set({ data: { height: 1 } } as Crypto.IBlock)).toThrow();
	});
	...
	it("should return whether the store contains a specific block", () => {
		...
		expect(store.has({ height: 1, id: "1" } as Crypto.IBlockData)).toBe(true);
		expect(store.has({ height: 5, id: "5" } as Crypto.IBlockData)).toBe(false);
	});
	...
	it("should delete blocks", () => {
		...
		store.delete({ height: 4, id: "4" } as Crypto.IBlockData);
		expect(store.count()).toBe(3);
		expect(store.has({ height: 4, id: "4" } as Crypto.IBlockData)).toBe(false);
	});
