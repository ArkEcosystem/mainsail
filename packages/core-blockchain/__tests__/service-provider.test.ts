...
describe("ServiceProvider", () => {
	...
	beforeEach(() => {
		...
		app.bind(Identifiers.StateStore).toConstantValue({ reset: jest.fn() });
		app.bind(Identifiers.DatabaseService).toConstantValue({});
		app.bind(Identifiers.DatabaseInteraction).toConstantValue({});
		app.bind(Identifiers.DatabaseBlockRepository).toConstantValue({});
		app.bind(Identifiers.TransactionPoolService).toConstantValue({});
	...
	describe("register", () => {
		it("should bind blockchain, state machine and block processr", async () => {
			...
			expect(app.isBound(Identifiers.StateMachine)).toBeFalse();
			expect(app.isBound(Identifiers.BlockchainService)).toBeFalse();
			expect(app.isBound(Identifiers.BlockProcessor)).toBeFalse();

			await serviceProvider.register();

			expect(app.isBound(Identifiers.StateMachine)).toBeTrue();
			expect(app.isBound(Identifiers.BlockchainService)).toBeTrue();
			expect(app.isBound(Identifiers.BlockProcessor)).toBeTrue();
		});
	});
	...
	describe("boot", () => {
		it("should call boot on blockchain service", async () => {
			...
			app.bind(Identifiers.BlockchainService).toConstantValue(blockchainService);
	...
	describe("configSchema", () => {
		...
		it("should allow configuration extension", async () => {
			jest.resetModules();
			const { defaults } = await import("@packages/core-blockchain/source/defaults");
