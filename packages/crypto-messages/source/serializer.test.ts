...
import { precommitData, prevoteData, proposalData, serializedPrecommit, serializedPrevote, serializedProposal } from "../test/fixtures/proposal";
...
describe<{
    ...
    it("#serializeProposal - should correctly serialize", async ({ serializer }) => {
        assert.equal(
            await serializer.serializeProposal(proposalData),
    ...
    it("#serializePrecommit - should correctly serialize", async ({ serializer }) => {
        assert.equal(
            await serializer.serializePrecommit(precommitData),
    ...
    it("#serializePrevote - should correctly serialize", async ({ serializer }) => {
        assert.equal(
            await serializer.serializePrevote(prevoteData),
