...
describe<{
	app: Application;
	databaseService: Contracts.Database.DatabaseService;
}>("DatabaseService - storage full", ({ it, beforeEach, assert }) => {
	const TOTAL_BLOCKS = 400;
	const BLOCK_BYTES = 1024; // each commit serializes to a fixed 1 KiB

	// Fixed-size serialized payload so every commit costs exactly BLOCK_BYTES against the budget.
	const serialized = "ab".repeat(BLOCK_BYTES);

	beforeEach((context) => {
		context.app = new Application();

		// Storage hands back every commit in the requested range; the byte budget is applied by
		// readCommits, which is the behaviour under test here.
		const storage = {
			getCommitsByBlockRange: (from: number, end: number) => {
				const data: { blockNumber: number }[] = [];
				for (let n = from; n <= Math.min(end, TOTAL_BLOCKS - 1); n++) {
					data.push({ blockNumber: n });
				}
				return data;
			},
		};
		...
		const commitFactory = {
			fromStorage: (data: { blockNumber: number }) => ({
				block: { number: data.blockNumber },
				serialized,
			}),
		};

		context.app.bind(Identifiers.Evm.Instance).toConstantValue(storage).whenTagged("instance", "evm");
		context.app.bind(Identifiers.Cryptography.Commit.Factory).toConstantValue(commitFactory);
		context.app.bind(Identifiers.Cryptography.Block.Factory).toConstantValue({});
		context.app.bind(Identifiers.Cryptography.Transaction.Factory).toConstantValue({});

		context.databaseService = context.app.resolve(DatabaseService);
	});

	it("short-circuits at maxBytes (yields < 400) and resumes until all blocks are read", async ({
		databaseService,
	}) => {
		// A budget that fits ~150 of the 400 requested blocks, deliberately not a whole multiple of the
		// block size so a commit straddles the limit — verifying the read stops *before* overfetching.
		const maxBytes = 150 * BLOCK_BYTES + BLOCK_BYTES / 2;

		const blockNumbers: number[] = [];
		let from = 0;
		let batches = 0;

		// Keep reading from where we left off until the whole range has been retrieved.
		while (blockNumbers.length < TOTAL_BLOCKS) {
			const batch: Contracts.Crypto.Commit[] = [];
			for await (const commit of databaseService.readCommits(from, TOTAL_BLOCKS - 1, maxBytes)) {
				batch.push(commit);
			}

			assert.true(batch.length > 0); // progress is guaranteed; the loop never stalls

			const batchBytes = batch.reduce((sum, commit) => sum + commit.serialized.length / 2, 0);
			assert.true(batchBytes <= maxBytes); // never overfetches past the budget

			// The very first request asked for all 400 but came back short because the budget was hit.
			if (batches === 0) {
				assert.true(batch.length < TOTAL_BLOCKS);
			}

			blockNumbers.push(...batch.map((commit) => commit.block.number));
			from += batch.length;
			batches++;
		}

		// Exactly the 400 blocks, contiguous and in order, with no gaps or duplicates.
		assert.true(batches > 1); // it genuinely took multiple round-trips
		assert.equal(
			blockNumbers,
			Array.from({ length: TOTAL_BLOCKS }, (_, index) => index),
		);
	});
});
