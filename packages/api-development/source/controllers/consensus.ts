	...
	public async state(request: Hapi.Request) {
		...
		return {
			data: {
				...
				lockedValue: state.lockedValue ? state.lockedValue.getProposal()?.data.block.header.id : null,
				validRound: state.validRound,
				validValue: state.validValue ? state.validValue.getProposal()?.data.block.header.id : null,
				// eslint-disable-next-line sort-keys-fix/sort-keys-fix
