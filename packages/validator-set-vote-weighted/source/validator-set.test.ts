...
describe<{
	...
	it("handleCommitBlock - should update ranking every full round", async ({ cryptoConfiguration, validatorSet }) => {
		...
		await validatorSet.onCommit({ getProposedCommitBlock: async () => ({ block: { header: { height: 0 } } }) } as Contracts.BlockProcessor.IProcessableUnit);
		assert.true(buildValidatorRankingSpy.calledOnce);

		let currentHeight = 0;
		for (let index = 0; index < activeValidators; index++) {
			await validatorSet.onCommit({ getProposedCommitBlock: async () => ({ block: { header: { height: currentHeight } } }) } as Contracts.BlockProcessor.IProcessableUnit);
