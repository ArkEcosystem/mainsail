...
export function buildProofOfPossession(secretKey: Uint8Array): {
	pk: Uint8Array;
	pop: Uint8Array;
} {
	// 1. Derive the compressed public key (48-byte G1).
	const pk = bls.getPublicKey(secretKey).toBytes();
	...
	const pop = bls.sign(messagePoint, secretKey).toBytes();
