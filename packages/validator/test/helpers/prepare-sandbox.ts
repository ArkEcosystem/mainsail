...
export const prepareSandbox = async (context: { sandbox?: Sandbox }): Promise<void> => {
	...
	await context.sandbox.app.resolve(BlockchainUtilities).register();
	...
	await context.sandbox.app.resolve(CoreCryptoProposal).register();
	...
	const workerPool = {
		getWorker: () => ({
			...
			consensusSignature: (method, message, privateKey) =>
				context
					.sandbox!.app.getTagged(Identifiers.Cryptography.Signature.Instance, "type", "consensus")!
