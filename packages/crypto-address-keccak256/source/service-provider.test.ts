...
describe<{
	...
	it("#register - should be ok", async ({ serviceProvider, app, validator }) => {
		...
		assert.true(app.isBound(Identifiers.Cryptography.Identity.Address.Size));
		assert.true(app.isBound(Identifiers.Cryptography.Identity.Address.Factory));
		assert.true(app.isBound(Identifiers.Cryptography.Identity.Address.Serializer));

		assert.true(validator.hasSchema("address"));
	});
});
