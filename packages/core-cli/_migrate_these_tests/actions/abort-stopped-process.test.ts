...
import { AbortStoppedProcess } from "@packages/core-cli/source/actions";
...
beforeEach(() => {
	...
	cli.app.rebind(Identifiers.AbortStoppedProcess).to(AbortStoppedProcess).inSingletonScope();
	action = cli.app.get(Identifiers.AbortStoppedProcess);
});
