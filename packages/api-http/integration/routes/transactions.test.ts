		...
		const testCases = [
			{
				...
				result: [...transactions].filter(tx => tx.type === 0).sort((a, b) => Number(b.blockHeight) - Number(a.blockHeight)),
			},
			{ path: "/transactions?type=1", result: [] },
			{
				path: "/transactions?type=4",
				result: [...transactions].filter(tx => tx.type === 4).sort((a, b) => Number(b.blockHeight) - Number(a.blockHeight)),
			}
		];
