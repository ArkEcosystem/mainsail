...
export class ApiContext {
    public constructor(private app: Application, private readonly apiHttp: CoreApiHttp, private readonly apiDatabase: CoreApiDatabase) { }
...
const setupDatabase = async (app: Application): Promise<CoreApiDatabase> => {
    const pluginConfiguration = app.get<Providers.PluginConfiguration>(Identifiers.PluginConfiguration).discover("@mainsail/api-database", "@mainsail/api-database").merge(
        {
            database: {
                ...databaseOptions,
...
const setupHttp = async (app: Application): Promise<CoreApiHttp> => {
    const pluginConfiguration = app.get<Providers.PluginConfiguration>(Identifiers.PluginConfiguration).discover("@mainsail/api-http", "@mainsail/api-http").merge(
        {
