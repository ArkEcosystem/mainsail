...
export class DelegatesController extends Controller {
    ...
    public async index(request: Hapi.Request) {
        ...
        const criteria = this.getQueryCriteria(request.query, delegateCriteriaSchemaObject) as Search.Criteria.DelegateCriteria;
        const options = this.getListingOptions();

        const wallets = await this.walletRepositoryFactory().findManyDelegatesByCritera(criteria, sorting, pagination, options);
    ...
    public async voters(request: Hapi.Request) {
        ...
        const pagination = this.getQueryPagination(request.query);
        const sorting = this.getListingOrder(request);
        const criteria = this.getQueryCriteria(request.query, walletCriteriaSchemaObject) as Search.Criteria.WalletCriteria;
        const options = this.getListingOptions();
