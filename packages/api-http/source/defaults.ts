...
import { Environment } from "@mainsail/kernel";
	...
	export const defaults = {
		...
		plugins: {
			...
			rateLimit: {
				blacklist: Environment.get<undefined>(Constants.EnvironmentVariables.CORE_API_RATE_LIMIT_BLACKLIST)?.split(",") ?? [],
				...
				whitelist: Environment.get<undefined>(Constants.EnvironmentVariables.CORE_API_RATE_LIMIT_WHITELIST)?.split(",") ?? [],
			},
			socketTimeout: 5000,
