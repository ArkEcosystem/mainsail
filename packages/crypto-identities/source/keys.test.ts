...
describe("Identities - Keys", () => {
	...
	describe("fromWIF", () => {
		it("should return two keys in hex", () => {
			const keys = Keys.fromWIF("SGq4xLgZKCGxs7bjmwnBrWcT4C1ADFEermj846KC97FSv1WFD1dA", devnet);
		...
		it("should return address", () => {
			const keys = Keys.fromWIF(data.wif, devnet);
			// @ts-ignore
			const address = Address.fromPublicKey(keys.publicKey.toString("hex"), devnet);
			expect(address).toBe(data.address);
		});
		...
		it("should get keys from compressed WIF", () => {
			const keys = Keys.fromWIF("SAaaKsDdWMXP5BoVnSBLwTLn48n96UvG42WSUUooRv1HrEHmaSd4", devnet);
		...
		it("should get keys from uncompressed WIF", () => {
			const keys = Keys.fromWIF("6hgnAG19GiMUf75C43XteG2mC8esKTiX9PYbKTh4Gca9MELRWmg", devnet);
