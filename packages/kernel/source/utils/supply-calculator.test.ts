...
describe<Context>("Supply Calculator - calculateSupply", ({ assert, beforeEach, it, each }) => {
    ...
    each("should calculate the genesis supply without milestone at height: ", ({ dataset, context }) => {
    ...
    each("should calculate the genesis supply with one milestone at height: ", ({ dataset, context }) => {
        ...
        const reward = (current) => {
            ...
            return (blockReward(2).times(7999)).plus(blockReward(3).times(current - 7999));
        };
    ...
    each("should calculate the genesis supply with four milestones at height: ", ({ dataset, context }) => {
        context.configuration.getMilestones()[1].height = 8000;
