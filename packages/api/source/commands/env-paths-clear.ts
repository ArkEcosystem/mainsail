...
export class Command extends Commands.Command {
	...
	public async execute(): Promise<void> {
		if (this.hasFlag("data") || this.hasFlag("all")) {
			await this.#clear("Data", this.app.get<Contracts.Paths>(Identifiers.Cli.Paths.Application).data);
		}

		if (this.hasFlag("config") || this.hasFlag("all")) {
			await this.#clear("Config", this.app.get<Contracts.Paths>(Identifiers.Cli.Paths.Application).config);
		}

		if (this.hasFlag("cache") || this.hasFlag("all")) {
			await this.#clear("Cache", this.app.get<Contracts.Paths>(Identifiers.Cli.Paths.Application).cache);
		}

		if (this.hasFlag("log") || this.hasFlag("all")) {
			await this.#clear("Log", this.app.get<Contracts.Paths>(Identifiers.Cli.Paths.Application).log);
		}
		if (this.hasFlag("temp") || this.hasFlag("all")) {
			await this.#clear("Temp", this.app.get<Contracts.Paths>(Identifiers.Cli.Paths.Application).temp);
		}
