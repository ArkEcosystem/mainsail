...
describe<{
...
}>("ApiStatusCommand", ({ beforeEach, it, assert, stub, clock }) => {
	beforeEach((context) => {
	...
	it("should render a table with the process information", async ({ processManager, cli }) => {
		// 1d 1h 1m 1s after pm_uptime, so the uptime cell is deterministic.
		...
		assert.true(
			["1", "mainsail-api", "1.0.0", "online", "1d 1h 1m 1s", "2%", "2.05 kB"].every((column) =>
